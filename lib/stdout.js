	...
	value)
{
	if (value instanceof Error) {
		const { message, name, stack, ...rest } = value;
			...
			// convert any errors to objects that will make it through the structured
			// cloning process when they're sent to the stdout extension
		return {
			__type: "Error",
			message,
			name,
	...
	options)
{
	...
	methods = Object.keys(console)
		...
		.reduce((result, method) => ({
			...result,
			[method](...args) {
				const payload = { method, filename, args: args.map(convertError) };
